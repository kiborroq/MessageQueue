package edu.school21.financialassistance.listener;
...
public class RabbitmqListener {
	...
	@RabbitListener(queues = "${edu.school21.amq.queue.financial-assistance}")
	public void onStudentInfoReceive(Message message) {
		try {
			...
			String filename = String.format("%s-%s_%d-course_%s_financial-assistance.pdf",
			...
			try (FileOutputStream fos = new FileOutputStream(path.toFile())) {
				freemarkerTemplateRender.renderPdf("financial-doc.ftl", Collections.singletonMap("student", studentInfo), fos);
				fos.flush();
			}
