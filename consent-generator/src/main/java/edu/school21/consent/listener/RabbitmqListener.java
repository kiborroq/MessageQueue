package edu.school21.consent.listener;
...
public class RabbitmqListener {
	...
	public void onStudentInfoReceive(Message message) {
		try {
			...
			String filename = String.format("%s-%s_%d-course_%s_consent.pdf",
			...
			try (FileOutputStream fos = new FileOutputStream(path.toFile())) {
				freemarkerTemplateRender.renderPdf("consent-doc.ftl", Collections.singletonMap("student", studentInfo), fos);
				fos.flush();
			}
